...
GENOME.class <<- readVCF('chr21.five.populations.vcf.gz',frompos = 1, topos = 100000,tid = 'chr21',numcols = 10000, gffpath = 'chr21.gtf')

#read data
#set.populations
getpositiondata <- function(x,y){
  GENOME.class <<- readVCF('chr21.five.populations.vcf.gz',frompos = x, topos = y,tid = 'chr21',numcols = 10000, gffpath = 'chr21.gtf')
...
SlidingWindow <- function(window_size,window_jump){
...
SlidingWindow(win_size,win_jump)
