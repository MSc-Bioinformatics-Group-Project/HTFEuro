    ...
    try:
        ...
        if request.method == 'POST':
            if form.validate_on_submit():
                ...
                return redirect(url_for("statspage",statistics = statistics, enterpop=enterpop))
        return render_template('genomic_view3.html', nsites=nsites, bisites=bisites ,psites=psites, pi = pi,form=form, statistics = statistics, enterpop=enterpop)
    except:
...
def statspage():
    ...
    return render_template("statistics.html",answer=answer, statistics=statistics, enterpop=enterpop)
